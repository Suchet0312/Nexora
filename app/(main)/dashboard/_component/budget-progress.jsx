...
export function BudgetProgress({ initialBudget, currentExpenses }) {
  ...
  // ✅ Trigger budget alert email via API when >90%
  useEffect(() => {
    const sendAlert = async () => {
      if (percentUsed <= 90 || !initialBudget?.userId) return;

      console.log("Triggering budget alert API...");

      try {
        const res = await fetch("/api/send-budget-alert", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            userId: initialBudget.userId,
        ...
        console.log("Budget alert API response:", data);
      } catch (err) {
        console.error("Failed to call budget alert API:", err);
      }
    };
