...
export default function EmailTemplate({
  ...
  // Fallback to preview data if none passed
  const safeData =
    data && Object.keys(data).length > 0
      ? data
      : PREVIEW_DATA[type]?.data || {};

  const safeUserName = userName ?? PREVIEW_DATA[type]?.userName ?? "User";

  if (type === "monthly-report") {
    const income = safeData?.stats?.totalIncome ?? 0;
    const expenses = safeData?.stats?.totalExpenses ?? 0;
    const categories = safeData?.stats?.byCategory || {};
      ...
      <Html>
        ...
        <Body style={styles.body}>
          <Container style={styles.container}>
            ...
            <Text style={styles.text}>Hello {safeUserName},</Text>
            <Text style={styles.text}>
              Here&rsquo;s your financial summary for{" "}
              {safeData?.month || "this month"}:
            </Text>
...
// ✅ Attach PreviewProps so playground uses PREVIEW_DATA
